...
if sys.platform == "darwin":
    print(json.dumps({"ok": False, "error": "Excel adapter is not supported on macOS. The bridge uses Windows COM automation which is not available on Mac."}), file=sys.stderr)
    sys.exit(1)
...
def run_bridge():
    parser = argparse.ArgumentParser(
        description="Run Python code against a live Microsoft Excel COM instance."
    )
    ...
    parser.add_argument(
        ...
        help="Excel COM ProgID. Defaults to Excel.Application.",
    )
    parser.add_argument(
        "--allow-launch",
        action="store_true",
        help="Launch Excel if no running process is available.",
    )
    ...
    try:
        ...
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            exec(compile(code, "<excel_bridge>", "exec"), namespace, namespace)
