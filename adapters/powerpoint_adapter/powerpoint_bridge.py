...
if sys.platform == "darwin":
    print(json.dumps({"ok": False, "error": "PowerPoint adapter is not supported on macOS. The bridge uses Windows COM automation which is not available on Mac."}), file=sys.stderr)
    sys.exit(1)
...
def run_bridge():
    parser = argparse.ArgumentParser(
        description="Run Python code against a live Microsoft PowerPoint COM instance."
    )
    ...
    parser.add_argument(
        "--app-progid",
        default="PowerPoint.Application",
        help="PowerPoint COM ProgID. Defaults to PowerPoint.Application.",
    )
    parser.add_argument(
        "--allow-launch",
        action="store_true",
        help="Launch PowerPoint if no running process is available.",
    )
    ...
    try:
        ...
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            exec(compile(code, "<powerpoint_bridge>", "exec"), namespace, namespace)
