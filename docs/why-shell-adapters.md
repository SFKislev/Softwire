...
The Wire connects those two worlds. The agent keeps using the shell; the bridge
...
adapter can use `creative_adapters/com_bridge.py`. This is the simplest communication path.

When an app requires in-process execution, the adapter can ship a small in-app
extension and use `creative_adapters/local_http_bridge.py` with a tokenized
