...
def _write_session_file(port):
    ...
    payload = {
        "app": SESSION_NAME,
        "url": "http://{}:{}/eval".format(HOST, port),
        "host": HOST,
        "port": port,
        "token": _token,
...
class BridgeHandler(BaseHTTPRequestHandler):
    server_version = "CreativeAdapterHoudini/0.1"
    ...
    def do_POST(self):
        ...
        if not task.event.wait(timeout=30):
            _json_response(self, 504, {"ok": False, "error": "Houdini script timed out"})
            return
...
def start_bridge():
    global _server, _server_thread, _token, _event_loop_registered
    if _server:
        return
    ...
    _server_thread = threading.Thread(
        ...
        name="CreativeAdapterHoudiniBridge",
        daemon=True,
    )
    _server_thread.start()
    _write_session_file(_server.server_address[1])

    if not _event_loop_registered:
        hou.ui.addEventLoopCallback(_process_tasks)
        _event_loop_registered = True

    try:
        hou.ui.setStatusMessage(
            "Creative Adapter Bridge listening on {}".format(_server.server_address[1])
        )
    except Exception:
        pass
...
def stop_bridge():
    global _server, _server_thread, _token, _event_loop_registered
    if _event_loop_registered:
        try:
            hou.ui.removeEventLoopCallback(_process_tasks)
        except Exception:
            pass
        _event_loop_registered = False
    if _server:
