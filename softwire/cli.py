...
def install_codex_docs(args):
    ...
    ok = copy_dir(source, target_root / "softwire", force=args.force)
...
def install_claude_docs(args):
    ...
    target = target_root / "softwire.md"
...
def install_cursor_docs(args):
    ...
    target_root = Path(args.path).expanduser() if args.path else Path.home() / ".cursor" / "skills"
    return copy_dir(source, target_root / "softwire", force=args.force)


def install_kilo_docs(args):
    source = agent_docs_dir() / "kilo"
    target_root = Path(args.path).expanduser() if args.path else Path.home() / ".kilo" / "skills"
    return copy_dir(source, target_root / "softwire", force=args.force)


def install_qwen_docs(args):
    target_root = Path(args.path).expanduser() if args.path else Path.home() / ".qwen"
    target = target_root / "softwire.md"
    source = agent_docs_dir() / "common.md"
    if not copy_file(source, target, force=args.force):
        return False
...
def install_opencode_docs(args):
    ...
    return copy_file(source, target_root / "softwire.md", force=args.force)
...
def cmd_setup(args):
    ...
    print("Verify the bridge runtime with: softwire adapters")
    return 0 if ok else 1
...
def build_parser():
    ...
    setup_parser = subparsers.add_parser(
        "setup",
        help="Register SoftWire with global agent instruction locations.",
    )
    ...
    install_agent_docs_parser = subparsers.add_parser(
        "install-agent-docs",
        help="Install modular agent instructions for supported harnesses or generic AGENTS.md users.",
    )
