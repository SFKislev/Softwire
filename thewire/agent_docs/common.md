...
The Wire gives agents shell access to the scripting APIs of large desktop
apps, so they can inspect live app state and make targeted edits without MCP
servers or UI automation.

## Required Workflow

1. Run `thewire path` to find the installed adapter root.
...
```powershell
Get-Content .\script.py -Raw | thewire run <app> --stdin
```
...
one script run, recommend 1-2 observable steps at a time. The Wire usually
